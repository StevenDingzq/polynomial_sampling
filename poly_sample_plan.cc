#include "poly_sample_plan.h"
#include <pangolin/pangolin.h>
#include <unistd.h>

static void DrawLine2D(double x1, double y1, double x2, double y2, float r,
                       float g, float b)
{
  glColor3f(r, g, b);
  glBegin(GL_LINES);
...
static void DrawSimpleRectangle(double left, double top, double right,
                                double bottom, float r, float g, float b)
{
  glColor3f(r, g, b);
  glBegin(GL_LINES);
...
void Road::showRoad(Point p)
{
  ...
  glBegin(GL_LINES);
  glLineWidth(3);   // 设置线宽
  DrawLine2D(0. - RWidth, 0.0, 0. - RWidth, Sheight, 0.f, 0.8f, 0.f);
  DrawLine2D(RWidth, 0.0, RWidth, Sheight, 0.f, 0.8f, 0.f);
...
// 创建向量
void Mat::createVector(double tmp1, double tmp2, double tmp3, double tmp4,
...
// 求线性方程组的解(ax=b ,求 x)，a 为方阵 ，b 为列向量 //矩阵 a
// 为方阵并且方程组有唯一解时返回 true
bool Mat::solve(Mat a, Mat b)
{
  ...
  for (int i = 1; i <= aa.m; i++) {
    ...
    if (k <= aa.m) {   // 说明第 i 列有不为0的元素
      // 交换第 i 行和第 k 行所有元素
      for (int j = i; j <= aa.n;
           j++) {   // 从第 i 个元素交换即可，因为前面的元素都为0
        aa.mat[0][j] = aa.mat[i][j];
        aa.mat[i][j] = aa.mat[k][j];
        aa.mat[k][j] = aa.mat[0][j];   // 使用aa.mat[0][j]作为中间变量交换元素
      }
      double c;   // 倍数
    ...
    } else {   // 没有找到则说明系数矩阵秩不为 n
               // ，说明方程组中有效方程的个数小于 n
      cout << "系数矩阵奇异，线性方程组无解或有无数解" << endl;
      return false;
    }
  }
  cout << "aa upper rectangle:\n";
  aa.PrintMat();
...
{
  if (state != DriveStatus::turning || camera_p == nullptr ||
      view_p == nullptr) {
    return;
  }
  glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
  ...
  if (curTrackIndex == -1) {   // 刚开始进入转弯状态时，计算轨迹
    ...
    for (double t = t0; t <= t1; t += delta_t) {   // 根据五次多项式计算轨迹
      double x_tmp = A.mat[1][1] * pow(t, 5) + A.mat[2][1] * pow(t, 4) +
                     A.mat[3][1] * pow(t, 3) + A.mat[4][1] * pow(t, 2) +
                     A.mat[5][1] * pow(t, 1) + A.mat[6][1];
      double y_tmp = B.mat[1][1] * pow(t, 5) + B.mat[2][1] * pow(t, 4) +
                     B.mat[3][1] * pow(t, 3) + B.mat[4][1] * pow(t, 2) +
                     B.mat[5][1] * pow(t, 1) + B.mat[6][1];
      Point ptmp;
      ptmp.x = x_tmp;
      ptmp.y = y_tmp;
  ...
  curTrackIndex++;
  // cout << "curTrackIndex = " << curTrackIndex << ", trackPoints.size() = " <<
  // trackPoints.size() << endl;
...
{
  if (state != DriveStatus::exiting || camera_p == nullptr ||
      view_p == nullptr) {
    return;
  }
  glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
  pangolin::OpenGlRenderState camera(
      ...
      pangolin::ModelViewLookAt(Swidth / 2.0, 0.0, 1800, 0, Sheight / 2, 0,
                                pangolin::AxisY));
  ...
  for (double t = t0; t <= t1; t += delta_t) {
    double vx_tmp = 5 * A.mat[1][1] * pow(t, 4) + 4 * A.mat[2][1] * pow(t, 3) +
                    3 * A.mat[3][1] * pow(t, 2) + 2 * A.mat[4][1] * pow(t, 1) +
                    A.mat[5][1];
    double vy_tmp = 5 * B.mat[1][1] * pow(t, 4) + 4 * B.mat[2][1] * pow(t, 3) +
                    3 * B.mat[3][1] * pow(t, 2) + 2 * B.mat[4][1] * pow(t, 1) +
                    B.mat[5][1];
    glColor3f(1.0, 0., 0.);
    glPointSize(5.0);
    glBegin(GL_POINTS);
  ...
  // 同时绘制x-t和y-t加速度曲线
  for (double t = t0; t <= t1; t += delta_t) {
    double ax_tmp = 20 * A.mat[1][1] * pow(t, 3) +
                    12 * A.mat[2][1] * pow(t, 2) + 6 * A.mat[3][1] * pow(t, 1) +
                    2 * A.mat[4][1];
    double ay_tmp = 20 * B.mat[1][1] * pow(t, 3) +
                    12 * B.mat[2][1] * pow(t, 2) + 6 * B.mat[3][1] * pow(t, 1) +
                    2 * B.mat[4][1];
    glColor3f(0., 1., 0.);
    glPointSize(5.0);
    glBegin(GL_POINTS);
...
void Polynomial::SetPangolinParams(pangolin::OpenGlRenderState *camera,
                                   pangolin::View *view)
{
  camera_p = camera;
  view_p = view;
 ...
 * 图形化工具从easyX改为pangolin,在Ubuntu系统下开发、验证,
 * 采用状态机控制整个仿真过程
 ...
 *
 * 共12个方程，其中12个未知数（a0 - a5, b0 -
 * b5），可以通过线性方程组求出唯一解， 即： X = A * T，Y = B * T； 求解A、B。
...
{
  // 初始化图形窗口
  pangolin::CreateWindowAndBind("quintic", Swidth, Sheight);
  glEnable(GL_DEPTH_TEST);

  // Define Projection and initial ModelView matrix
  pangolin::OpenGlRenderState camera(
      // 图像宽度、高度、4个内参fx fy cx cy以及最近、最远视距
      pangolin::ProjectionMatrix(Swidth, Sheight, 1500, 1500, Swidth / 2,
                                 Sheight / 2, 0.1, 2500),
      /*相机的初始坐标，相机视点的初始位置（即相机光轴朝向），相机轴方向*/
      pangolin::ModelViewLookAt(0, Sheight / 2, 1800, 0, Sheight / 2, 0,
                                pangolin::AxisY));
